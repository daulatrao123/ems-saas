...
export function LastResponse({ last, row }: { last: LR | null; row: CommandRow | null }) {
  return (
      ...
      {!last && !row ? <div className="mt-3 text-sm text-gray-500">No command issued in this session.</div> : (
        <>
          <div className="mt-2 text-base font-bold text-white">
            {COMMAND_LABEL[(row?.command || (last as LR).command)] || (row?.command || (last as LR).command).toUpperCase()}
