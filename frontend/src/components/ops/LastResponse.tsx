"use client";
import { COMMAND_LABEL, CommandRow, LastResponse as LR, TERMINAL, fmtDateTime, statusTone } from "./types";
...
export function LastResponse({ last, row: storedRow, scope = "last-response", embedded = false }: { last: LR | null; row: CommandRow | null; scope?: string; embedded?: boolean }) {
  ...
  return (
    <section data-testid={scope} className={embedded ? "border-t border-[#1e2a3a] pt-3 min-w-0" : `${panel} p-4`}>
      <div data-testid={`${scope}-heading`} className={label}>{embedded ? "Last recorded wing command" : "Last recorded device command"}</div>
      {!last && !row ? <div data-testid={`${scope}-empty`} className="mt-2 text-sm text-gray-500">{embedded ? "—" : "No device-level command issued."}</div> : (
        <>
          <div data-testid={`${scope}-context`} className="mt-3 text-[11px] text-gray-400">{row && TERMINAL.has(row.status) ? "Historical command result" : row ? "Command awaiting completion" : "Latest submission"}{row?.created_at ? ` · ${fmtDateTime(row.created_at)}` : ""}</div>
          ...
          <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1.5 font-mono text-[11px]">
            {row ? (
              <>
                ...
                <Row scope={scope} k="REQUESTED" v={fmtDateTime(row.created_at)} />
                <Row scope={scope} k="DELIVERED" v={fmtDateTime(row.delivered_at)} />
                {row.hardware_verified_at && <Row scope={scope} k={verificationLabel} v={fmtDateTime(row.hardware_verified_at)} tone={contactorVerified ? "text-emerald-300" : "text-gray-300"} />}
                <Row scope={scope} k="COMPLETED" v={fmtDateTime(row.completed_at)} />
              ...
              <>
                ...
                <Row scope={scope} k="SENT" v={fmtDateTime(last.at)} />
              </>
