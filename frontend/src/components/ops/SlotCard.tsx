...
type Props = { device: Device; code: string; slot: Slot; queue: QueueFn; setSlotConfig: SlotConfigFn; isPending: (d: string, c: string, s?: string) => boolean; readOnly: boolean; lastCmd?: CommandRow };

const telemetry = (v: string | undefined, offline: boolean) => (offline ? "UNKNOWN (offline)" : (v || "UNKNOWN").toUpperCase());

export function SlotCard({ device, code, slot, queue, setSlotConfig, isPending, readOnly, lastCmd }: Props) {
  ...
  // Read-only Pi telemetry (never a control): toggle_input = physical toggle GPIO, physical_toggle = contactor feedback.
  const toggleIn = telemetry(slot.toggle_input, !device.connected);
  const contactor = telemetry(slot.physical_toggle, !device.connected);
  ...
  return (
    <section data-testid={`slot-card-${code}`} className={`${panel} ${active ? "border-emerald-500/40" : ""} p-4 flex flex-col gap-3`}>
      ...
      <dl className="grid grid-cols-2 gap-x-3 gap-y-1 font-mono text-[10px]">
        <dt className="text-gray-500">PHYSICAL TOGGLE</dt><dd data-testid={`slot-toggle-input-${code}`} className={toggleIn === "ON" ? "text-emerald-300" : toggleIn === "OFF" ? "text-gray-300" : "text-gray-500"}>{toggleIn}</dd>
        <dt className="text-gray-500">CONTACTOR</dt><dd data-testid={`slot-physical-${code}`} className={contactor === "ON" ? "text-emerald-300" : contactor === "OFF" ? "text-gray-300" : "text-gray-500"}>{contactor}</dd>
      ...
      {!readOnly && (
        <div className="flex items-center justify-between gap-2 border-t border-[#1e2a3a] pt-3">
          <span className={label}>Logical slot</span>
