...
export function useCalendarComparison(societyId: string, summary: EnergySummary | null, month: string, revision: number) {
  ...
  useEffect(() => {
    ...
    readRequest(`/api/energy/graph/comparison?${q}`, controller.signal)
      .then((response) => {
        const data = response.data as CalendarComparison;
        ...
        const valid = data?.device_id === device && data.mode === mode && data.version === version && data.operating_date === day
          ...
          && scopeValid(data) && rowsValid(data.society?.rows) && WINGS.every((wing) => data.wings?.[wing]?.wing === wing && rowsValid(data.wings[wing].rows));
      ...
      }).catch((e) => { if (!controller.signal.aborted) setResult({ key, data: null, error: `Month comparison unavailable: ${errorText(e).detail}` }); });
