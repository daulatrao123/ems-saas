...
export function GenerationCard({ meter, monthly, operatingDate, resetPeriod, activeGenerationWing }: Props) {
  ...
  return (
    <section data-testid="energy-generation-card" className="border-y border-[#293443] py-5 flex flex-col gap-5">
      ...
      <dl className="ops-summary-grid font-mono text-[11px]">
        <div><dt className="text-gray-500">TODAY</dt><dd data-testid="energy-generation-today" className={`mt-2 ${today == null ? "ops-value-unavailable" : "ops-value text-emerald-300"}`}>{fmtKwh(today)}</dd></div>
        <div><dt className="text-gray-500">POWER NOW</dt><dd data-testid="energy-generation-power" className="mt-2 ops-value text-gray-300">{fmtKw(online ? meter.current_power_kw : null)}</dd></div>
        <div><dt className="text-gray-500">RESET PERIOD {resetPeriod}</dt><dd data-testid="energy-generation-period" className={`mt-2 ${period == null ? "ops-value-unavailable" : "ops-value text-white"}`}>{fmtKwh(period)}</dd></div>
        <div><dt className="text-gray-500">UNATTRIBUTED TODAY</dt><dd data-testid="energy-generation-unattributed" className={`mt-2 ${unattr == null ? "ops-value-unavailable" : "ops-value text-amber-300"}`}>{fmtKwh(unattr)}</dd></div>
      </dl>
      ...
      <details data-testid="energy-monthly-details" className="ops-disclosure">
        <summary data-testid="energy-monthly-details-toggle" className="text-sm font-medium">Monthly physical generation history<span data-testid="energy-monthly-window" className="ml-auto text-xs text-gray-400">Last 6 months</span></summary>
