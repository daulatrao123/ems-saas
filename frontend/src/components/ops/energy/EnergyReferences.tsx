...
export function EnergyReferences({ societyId, summary, readOnly, refresh, children }: {
  societyId: string; summary: EnergySummary; readOnly: boolean; refresh: () => void; children: ReactNode;
}) {
  ...
  return <BillHistoryContext.Provider value={{ open: setEditor, readOnly }}>
    {children}
    <section data-testid="historical-consumption-section" className="border-t border-[#1e2a3a] pt-4 space-y-4">
      <h2 data-testid="historical-consumption-title" className="text-base font-bold text-white">Historical Consumption Baseline</h2>
