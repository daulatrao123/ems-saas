...
export function SocietyEnergyComparison({ data, excessEnabled }: { data: EnergyComparison | null; excessEnabled: boolean }) {
  ...
  return <section data-testid="society-energy-comparison" className="border-t border-[#1e2a3a] pt-4 space-y-3">
    ...
    <ConsumptionScopeLabel data={data} scope="society-comparison" />
    <dl className="flex flex-wrap gap-x-10 gap-y-3 text-xs">
      ...
      <div><dt className="text-gray-500">Included-wing consumption · {data.mode === "MANUAL" ? "bill-derived daily reference" : "physical today"}</dt><dd data-testid="society-comparison-consumption" className="mt-1 text-amber-300">{fmtKwh(point.consumed_kwh)}</dd></div>
