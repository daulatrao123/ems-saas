...
export function BillMonthEditor({ societyId, deviceId, wing, endMonth, readOnly, onSaved }: {
  societyId: string; deviceId: string; wing: WingCode; endMonth: string; readOnly: boolean; onSaved: (history: BillHistory) => void;
}) {
  ...
  useEffect(() => {
    ...
    api.get(`/api/energy/bills?${new URLSearchParams({ society_id: societyId, device_id: deviceId, wing, end_month: endMonth })}`, { signal: controller.signal })
      .then(({ data }: { data: BillHistory }) => {
        if (!controller.signal.aborted && data.device_id === deviceId && data.wing === wing) {
  ...
  return <form data-testid="bills-month-form" className="space-y-3" onSubmit={async (event) => {
    ...
    {!history ? <p data-testid="bills-loading" className="text-gray-400">{error ? "History unavailable" : "Loading history…"}</p> : <>
      ...
      <div data-testid="bills-saved-summary" className="text-xs text-cyan-200">Saved reference: {dailyRate(history.reference_daily_kwh)} · {history.valid_months} valid months · HISTORICAL</div>
