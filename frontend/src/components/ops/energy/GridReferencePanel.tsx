...
export function GridReferencePanel({ societyId, deviceId, grid, allocation: a, readOnly, onSaved }: {
  ...
  return <section data-testid="grid-reference-panel" className="border-t border-[#1e2a3a] pt-4 space-y-4">
    ...
    <details data-testid="allocation-calculation-details" className="ops-disclosure text-xs">
      <summary data-testid="allocation-calculation-details-toggle" className="text-sm font-medium"><span>Quota & Grid calculation breakdown</span><span data-testid="allocation-calculation-summary" className="text-xs text-gray-400 ml-auto">{a.status} · {a.reason.replaceAll("_", " ")}</span></summary>
    <div data-testid="allocation-reference-flow" className="space-y-4 py-3 text-xs font-mono">
      ...
      <div aria-hidden="true" className="text-gray-600">↓</div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{a.wings.map((w) => <div key={w.wing} data-testid={`quota-step-${w.wing}`} className="border-l-2 border-cyan-800 pl-3 pb-3 space-y-1">
