...
export function MonthlyBars({ rows, unit }: { rows: MonthRow[]; unit: string }) {
  ...
  return (
    <svg data-testid="energy-monthly-chart" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMinYMid meet" className="w-full max-w-3xl h-44 font-mono" role="img" aria-label={`Monthly generation, last ${rows.length} months`}>
      {ticks.map((t) => (
