...
export type ConsumptionScope = { included_wings?: WingCode[]; excluded_wings?: WingCode[]; scope_basis?: string };
export type EnergyComparison = ConsumptionScope & { device_id: string; mode: CalculationMode; version: number; operating_date: string; consumption_basis: string; wings: Record<WingCode, ComparisonSeries & { wing: WingCode }>; society: ComparisonSeries };
...
export type EnergySummary = { device_id: string; as_of_operating_date: string; manual_generation_operating_date: string | null; reset_day: number; reset_period: string; generation_meter: GenerationMeter; wings: Record<string, WingSummary>; calculation: Calculation; references: EnergyReferenceData; clock_report?: unknown; target_delivery?: unknown };
