...
export default function SuperAdminDashboard() {
  ...
  const deleteSoc = async (id: string) => {
    ...
    try {
      await api.post("/api/super-admin/societies/delete", { id });
      fetchData();
  ...
  const deleteUser = async (id: string) => {
    ...
    try {
      await api.post("/api/super-admin/users/delete", { id });
      fetchData();
      showToast("Deleted", true);
    } catch { showToast("Failed", false); }
  };
  ...
  const saveFw = async () => {
    ...
    try {
      await api.post("/api/super-admin/firmware/save", fwForm);
      setFwForm({ version: "", code: "", changelog: "", forced: false });
      fetchData();
      showToast("Firmware saved", true);
    } catch { showToast("Failed", false); }
  };

  const deleteFw = async (v: string) => {
    if (!confirm("Delete version " + v + "?")) return;
    try {
      await api.post("/api/super-admin/firmware/delete", { version: v });
      fetchData();
      showToast("Deleted", true);
    } catch { showToast("Failed", false); }
  };

  const forceFw = async (v: string) => {
    try {
      await api.post("/api/super-admin/firmware/force", { version: v });
      fetchData();
      showToast(v + " forced", true);
    } catch { showToast("Failed", false); }
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-6 pt-20" style={{ background: "#0a0e17" }}>
          ...
          <div className="flex gap-2">
            {tab === "societies" && (
              <button
                ...
                className="px-4 py-2 bg-cyan-500 text-black text-xs font-bold rounded-lg hover:bg-cyan-600"
              >
                + Add Society
              </button>
            )}
            {tab === "users" && (
              <button
                onClick={() => {
                  setUserForm({ email: "", name: "", password: "", role: "society_admin", society_id: societies[0]?.id || "" });
                  setShowUserModal(true);
                }}
                className="px-4 py-2 bg-emerald-500 text-black text-xs font-bold rounded-lg hover:bg-emerald-600"
              >
                + Add User
              </button>
            )}
        ...
        <div className="flex gap-1 mb-6 bg-gray-900 p-1 rounded-lg w-fit">
          {tabs.map((t) => (
            <button
              ...
              className={"px-4 py-2 rounded-md text-xs font-semibold transition-all " + (tab === t.key ? "bg-cyan-500/20 text-cyan-400 border border-cyan-500/30" : "text-gray-500 hover:text-gray-300 border border-transparent")}
            >
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-gray-500 border-b border-gray-800">
                    ...
                    <th className="text-left px-4 py-2">Pi Device</th>
                    <th className="text-left px-4 py-2">Status</th>
                    <th className="text-left px-4 py-2">FW</th>
                    <th className="text-left px-4 py-2">Wing</th>
                  ...
                  {societies.map((s) => (
                    <tr key={s.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      ...
                      <td className="px-4 py-3">
                        <span className={"px-2 py-0.5 rounded-full text-[9px] font-bold " + (s.plan === "Professional" ? "bg-cyan-500/15 text-cyan-400" : "bg-gray-700 text-gray-400")}>{s.plan}</span>
                      </td>
                      <td className="px-4 py-3 text-gray-500 font-mono">{s.society_code || "--"}</td>
                      <td className="px-4 py-3">
                        {s.firmware_version ? (
                          <span className="px-2 py-1 rounded bg-emerald-500/10 text-emerald-400 text-[9px] font-bold border border-emerald-500/20">Linked</span>
                        ) : (
                          <span className="px-2 py-1 rounded bg-gray-700 text-gray-500 text-[9px] font-bold">No Device</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <span className={"flex items-center gap-1 " + (s.pi_online ? "text-emerald-400" : "text-red-400")}>
                          <span className={"w-1.5 h-1.5 rounded-full " + (s.pi_online ? "bg-emerald-400" : "bg-red-400")} />
                          {s.pi_online ? "Online" : "Offline"}
                        </span>
                      </td>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-gray-500 border-b border-gray-800">
                    ...
                    <th className="text-left px-4 py-2">Society</th>
                    <th className="text-right px-4 py-2">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  {users.map((u) => (
                    <tr key={u.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      ...
                      <td className="px-4 py-3">
                        <span className={"px-2 py-0.5 rounded-full text-[9px] font-bold " + (u.role === "super_admin" ? "bg-amber-500/15 text-amber-400" : u.role === "society_admin" ? "bg-cyan-500/15 text-cyan-400" : "bg-gray-700 text-gray-400")}>
                          {u.role.replace("_", " ")}
                        </span>
                      </td>
          ...
          <div className="space-y-6">
            <div className="bg-gray-900/80 border border-gray-800 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-800 bg-gray-800/50">
                <h2 className="text-xs font-semibold text-gray-300">Upload Firmware</h2>
              </div>
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Version</label>
                    <input
                      className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-gray-200 focus:outline-none focus:border-cyan-500 font-mono"
                  ...
                  <div className="flex items-end">
                    <label className="flex items-center gap-2 cursor-pointer pb-2">
                      <input
                        type="checkbox"
                        checked={fwForm.forced}
                        onChange={(e) => setFwForm({ ...fwForm, forced: e.target.checked })}
                        className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-red-500 focus:ring-red-500"
                      />
                      <span className="text-xs text-red-400 font-semibold">Force update all Pis</span>
                    </label>
                  </div>
                </div>
                <div>
                  <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Changelog</label>
                  <textarea
                    className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-gray-200 focus:outline-none focus:border-cyan-500 h-16 resize-none"
                    placeholder="What changed..."
                    value={fwForm.changelog}
                    onChange={(e) => setFwForm({ ...fwForm, changelog: e.target.value })}
                  />
                </div>
                <div>
                  <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Python Code</label>
                  <textarea
                    className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-emerald-400 font-mono focus:outline-none focus:border-cyan-500 h-64 resize-y"
                    placeholder="Paste complete Pi firmware code..."
                    value={fwForm.code}
                    onChange={(e) => setFwForm({ ...fwForm, code: e.target.value })}
                  />
                </div>
                <div className="flex gap-2">
                  <button onClick={saveFw} className="px-6 py-2 bg-cyan-500 text-black text-xs font-bold rounded-lg hover:bg-cyan-600">Save Version</button>
                  <button onClick={() => setFwForm({ version: "", code: "", changelog: "", forced: false })} className="px-4 py-2 border border-gray-700 text-gray-500 text-xs rounded hover:border-gray-500">Clear</button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-900/80 border border-gray-800 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-800 bg-gray-800/50">
                <h2 className="text-xs font-semibold text-gray-300">All Versions</h2>
              </div>
                  ...
                  <div key={v.version} className="px-5 py-4 flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {v.forced && <span className="px-2 py-0.5 rounded-full bg-red-500/15 text-red-400 text-[9px] font-bold border border-red-500/30">FORCED</span>}
                        <span className="text-[9px] text-gray-600">{v.created_at ? new Date(v.created_at).toLocaleString() : ""}</span>
                      </div>
                      {v.changelog && <p className="text-[10px] text-gray-500">{v.changelog}</p>}
                    </div>
                    <div className="flex items-center gap-2 ml-4">
                      <button
                        onClick={() => forceFw(v.version)}
                        className={"px-3 py-1.5 text-[10px] font-semibold rounded " + (v.forced ? "bg-red-500/15 text-red-400 border border-red-500/30" : "border border-gray-700 text-gray-500 hover:border-amber-500 hover:text-amber-400")}
                      >
                        {v.forced ? "Forced" : "Force"}
                      </button>
                      <button
                        onClick={() => deleteFw(v.version)}
                        className="px-3 py-1.5 border border-gray-700 text-gray-500 text-[10px] rounded hover:border-red-500 hover:text-red-400"
                      >
                        Delete
                      </button>
                    </div>
            ...
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-gray-200 focus:outline-none focus:border-cyan-500" value={socForm.name} onChange={(e) => setSocForm({ ...socForm, name: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Location</label>
                  <input className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-gray-200 focus:outline-none focus:border-cyan-500" value={socForm.location} onChange={(e) => setSocForm({ ...socForm, location: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Plan</label>
                  <select className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-gray-200 focus:outline-none focus:border-cyan-500" value={socForm.plan} onChange={(e) => setSocForm({ ...socForm, plan: e.target.value })}>
                    <option>Basic</option>
                ...
                <div>
                  ...
                  <input className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-gray-200 focus:outline-none focus:border-cyan-500" value={socForm.society_code} onChange={(e) => setSocForm({ ...socForm, society_code: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Tailscale IP</label>
                  <input className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-gray-200 focus:outline-none focus:border-cyan-500" value={socForm.tailscale_ip} onChange={(e) => setSocForm({ ...socForm, tailscale_ip: e.target.value })} />
                </div>
              </div>
        ...
        {toast && (
          <div className={"fixed bottom-4 right-4 px-4 py-2 rounded-lg border z-50 " + (toast.ok ? "border-emerald-500/50 text-emerald-400" : "border-red-500/50 text-red-400") + " bg-gray-900"}>
            {toast.msg}
          </div>
        )}
      </main>
