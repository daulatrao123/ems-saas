"""EMS authoritative schema baseline (single Alembic root).

History note: before this revision the repository carried four competing
root migrations (0001_ems_current_schema, 0001_industrial_schema,
20260906_0001 and migrations/0001_industrial_baseline). None of them was ever
executed by the production start command (Render launched uvicorn directly),
so the production schema is whatever ``main.ensure_db_schema()`` produced.
This revision reproduces exactly that schema, idempotently, so it can:

* initialise a fresh PostgreSQL database, and
* adopt an existing v5/v6 production database (including legacy wing->slot
  naming) without data loss.

Databases stamped with one of the retired revision ids are re-stamped to this
revision by ``alembic/env.py`` (see LEGACY_REVISIONS there).
"""
...
def upgrade() -> None:
    ...
    # Legacy v5/v6 wing -> slot normalization (tables first, then columns).
    op.execute("""
    DO $$ BEGIN
    ...
    # Columns added across v5.6 -> v6.x on already-deployed databases.
    ...
    # v5.6 enforced one Pi per society; v6 allows several.
...
def downgrade() -> None:
    raise RuntimeError("Baseline downgrade is intentionally disabled; restore from a tested database backup instead.")
