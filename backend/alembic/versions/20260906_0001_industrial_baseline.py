"""Establish EMS schema outside application startup.

Legacy wing/slot names are normalized here. PostgreSQL DDL runs transactionally
under Alembic, so a failed migration rolls back instead of breaking app startup.
"""
...
def upgrade() -> None:
    op.execute("""
        CREATE TABLE IF NOT EXISTS societies (
            ...
            config_version INT DEFAULT 1, reset_day INT DEFAULT 15
        )
    """)
    op.execute("""
        CREATE TABLE IF NOT EXISTS users (
            id SERIAL PRIMARY KEY, email TEXT UNIQUE, name TEXT, password TEXT,
            role TEXT, society_id INT
        )
    """)
    op.execute("""
    ...
    # Preserve legacy data while normalizing names used by the current application.
    op.execute("""
        DO $$ BEGIN
    ...
    op.execute("""
        CREATE TABLE IF NOT EXISTS pi_events (
            ...
            type TEXT, message TEXT, FOREIGN KEY (device_id) REFERENCES pi_devices(id) ON DELETE CASCADE
        )
    """)
    op.execute("""
    ...
    op.execute("ALTER TABLE pi_commands ADD COLUMN IF NOT EXISTS delivered_at TIMESTAMPTZ")
    op.execute("ALTER TABLE pi_commands ADD COLUMN IF NOT EXISTS acked_at TIMESTAMPTZ")
    op.execute("ALTER TABLE pi_commands ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ")
    op.execute("ALTER TABLE pi_commands ADD COLUMN IF NOT EXISTS error TEXT")
    op.execute("ALTER TABLE pi_commands ADD COLUMN IF NOT EXISTS result TEXT")
    ...
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fk_users_society') THEN
                ALTER TABLE users ADD CONSTRAINT fk_users_society FOREIGN KEY (society_id)
                REFERENCES societies(id) ON DELETE SET NULL;
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fk_devices_society') THEN
                ALTER TABLE pi_devices ADD CONSTRAINT fk_devices_society FOREIGN KEY (society_id)
                REFERENCES societies(id) ON DELETE SET NULL;
            END IF;
        END $$;
    """)

    # Prevent old rows with NULL expiry from becoming immortal commands.
    op.execute("""
        UPDATE pi_commands
        SET expires_at = COALESCE(created_at, NOW()) + INTERVAL '300 seconds'
        WHERE expires_at IS NULL
    """)


def downgrade() -> None:
    # Deliberately non-destructive. Production data must not be dropped by rollback.
    pass
